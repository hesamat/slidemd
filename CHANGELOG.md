...
- **Global Ctrl+S** — the shortcut binding is now `global: true`; the browser's save dialog never appears in any editor window. In exported decks or embedded iframes the browser's native save is preserved. Ctrl+S in non-CodeMirror app inputs runs the app save while still suppressing the browser Save Page dialog.
...
- **Save preparation failures handled** — a throwing `_prepareSave` shows a toast and keeps the deck dirty instead of rejecting the shared in-flight promise unhandled.

### Editor Behaviour

- **Buffer flush on exit** — `toggleEditMode` flushes the pending debounced editor buffer before toggling, filing it under the slide it belongs to (including when a hidden slide causes navigation away); the sub-300 ms window where the last keystrokes could be lost on an immediate Ctrl+S is closed.
