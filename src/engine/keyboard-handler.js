...
export class KeyboardHandler {
    ...
    handleKeyboard(e) {
        ...
        if (this.actions.isImageSelected?.() && ["next", "prev", "first", "last", "reload"].includes(singleAction)) {
            return;
        }

        // If break mode is active, any navigation key ends the break
        if (this.actions.isBreakActive?.() && ["next", "prev", "first", "last", "goto"].includes(singleAction)) {
            e.preventDefault();
