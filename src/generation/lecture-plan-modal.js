      ...
      const collectPlan = () => ({
        ...
        rows: Array.from(rowsBody.querySelectorAll("tr")).map((rowEl, index) => ({
          ...
          durationMinutes: Math.max(
            1,
            parseInt(rowEl.querySelector("[data-field='duration']").value, 10) || 0,
          ),
          targetSlides: Math.max(
            0,
            parseInt(rowEl.querySelector("[data-field='slides']").value, 10) || 0,
          ),
      ...
      const renderValidation = () => {
        ...
        if (validation.valid && validation.warnings.length === 0) {
          validationEl.className =
            "lecture-plan-modal__validation lecture-plan-modal__validation--valid";
          validationEl.textContent = "Plan timing is balanced and ready for deck generation.";
        } else if (!validation.valid) {
          validationEl.className =
            "lecture-plan-modal__validation lecture-plan-modal__validation--error";
          validationEl.textContent = validation.errors.join(" ");
        } else {
          validationEl.className =
            "lecture-plan-modal__validation lecture-plan-modal__validation--warning";
