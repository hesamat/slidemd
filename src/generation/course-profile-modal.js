...
export class CourseProfileModal {
  ...
  static async showList() {
    ...
    return this._showListModal(profiles);
  }

  /**
   * Internal: render the list modal and return a Promise that resolves
   * when the user selects, creates, or cancels.
   */
  static _showListModal(profiles) {
    return new Promise((resolve) => {
      ...
      const cancelBtn = Array.from(
        backdrop.querySelectorAll(".course-profile-list-modal__actions .course-profile-modal__btn"),
      ).find(
        (btn) =>
          !btn.classList.contains("course-profile-list-modal__create") &&
          !btn.classList.contains("course-profile-list-modal__import"),
      );
      ...
      const handleProfileAction = async (profileId, action) => {
        ...
        if (action === "select") {
          ...
          const confirmed = await Notification.confirm(`Delete profile "${profile.name}"?`);
          if (confirmed) {
