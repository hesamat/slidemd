...
Return only the JSON block.`;
  ...
  static normalizePlan(rawPlan, defaults = {}) {
    const rowsSource = Array.isArray(rawPlan)
      ? rawPlan
      : Array.isArray(rawPlan?.rows)
        ? rawPlan.rows
        : [];
    ...
    return {
      ...
      totalMinutes: Math.max(
        1,
        parseInt(rawPlan?.totalMinutes, 10) || defaults.totalMinutes || this.sumMinutes(rows),
      ),
      breakCount: Math.max(
        0,
        parseInt(rawPlan?.breakCount, 10) ||
          defaults.breakCount ||
          rows.filter((row) => row.type === "break").length,
      ),
      activityCount: Math.max(
        0,
        parseInt(rawPlan?.activityCount, 10) || defaults.activityCount || 0,
      ),
      rows,
    };
  }
  ...
  static normalizeRow(row, index) {
    const rawType = String(row?.type || row?.kind || "concept")
      .trim()
      .toLowerCase();
    const type = this.normalizeType(rawType);
    const durationMinutes = Math.max(
      1,
      parseInt(row?.durationMinutes ?? row?.minutes ?? row?.duration, 10) ||
        this.getDefaultDuration(type),
    );
    const title =
      String(row?.title || this.getDefaultTitle(type, index)).trim() ||
      this.getDefaultTitle(type, index);
    const notes = String(
      row?.notes || row?.teachingGoal || row?.goal || row?.description || "",
    ).trim();
    ...
    return {
      ...
      targetSlides:
        type === "break"
          ...
          : Math.max(
              1,
              parseInt(row?.targetSlides ?? row?.slides ?? row?.slideCount, 10) ||
                this.getDefaultTargetSlides(type),
            ),
      notes,
    };
  }
  ...
  static validatePlan(plan, constraints = {}) {
    ...
    const totalMinutes = Math.max(
      0,
      parseInt(constraints.totalMinutes ?? plan?.totalMinutes, 10) || 0,
    );
    const expectedBreaks = Math.max(
      0,
      parseInt(constraints.breakCount ?? plan?.breakCount, 10) || 0,
    );
    ...
    if (totalMinutes > 0 && allocatedMinutes !== totalMinutes) {
      errors.push(
        `Allocated minutes (${allocatedMinutes}) must equal requested duration (${totalMinutes})`,
      );
    }

    if (actualBreaks !== expectedBreaks) {
      errors.push(
        `Expected ${expectedBreaks} break row${expectedBreaks === 1 ? "" : "s"}, found ${actualBreaks}`,
      );
    }
  ...
  static generateFallbackPlan(topic, options = {}) {
    ...
    const breakDuration =
      breakCount > 0
        ? Math.max(5, Math.min(10, Math.floor(totalMinutes / Math.max(5, breakCount + 4))))
        : 0;
