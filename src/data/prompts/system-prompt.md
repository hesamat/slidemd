...
- Use `::: table { ... }` to wrap a markdown table when it needs sizing, alignment, or styling. Never use raw `<table style="...">`. Attributes MUST be inside `{ }` braces using `key=value` syntax (NOT `key: value`). Example: `::: table { width=60 align=center }\n| A | B |\n| --- | --- |\n| 1 | 2 |\n:::`. The body must be a valid markdown table: it must start with a header row, followed by a `| --- | ... |` separator row, then data rows. Use `no-header` when the header should be hidden visually, but the source must still include the header row and `---` separator. Place the closing `:::` on its own line and leave a blank line after it before the next paragraph or element. Supported attributes: `width` (1–100, percentage of area), `align` (left|center|right, default center), `fontSize` (px), `columns` (relative weights, e.g. `2,1,3`), `borders=false`, `striped=false`, `headerColor` (hex color for the header row, e.g. `headerColor="#003C68"`), `no-header` (bare flag). Tables without a directive stay content-sized and centered. Preserve any existing `::: table` blocks exactly, including all attributes.
...
- Use KaTeX syntax for math: `$...$` for inline math and `$$...$$` for display math. The app renders these with KaTeX. Keep the `$` delimiters flush against the math (`$x$`, not `$ x $`), and put display math on its own line. In KaTeX, `_` is a subscript token even inside `\text{...}`, so escape every literal underscore with `\_` (e.g. `\text{land\_area\_km}`, not `\text{land_area_km}`). Do not write ASCII art equations or use `<sup>`/`<sub>` instead of proper KaTeX.
