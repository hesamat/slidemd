...
- Use `split` when a single source slide has enough material for multiple focused slides. A slide with 8+ bullets, a dense list plus a code example, or multiple distinct concepts crammed together is a strong split candidate. Try a multi-column layout first (`two-column`, `three-column`, or `::: text-block { column-count=N markdown=true }`); if the content is still too dense or covers genuinely distinct topics, split it into one slide per topic. Do not split a thin slide just to increase the slide count.
- Use `add` when the narrative is missing a transition, recap, example, or hook that does not exist in the source deck. Long decks often jump between major sections with no segue — an `add` transition slide between sections helps the audience follow the flow. A deck that ends without a recap or key-takeaways slide is another good `add` candidate. Do not add a slide that merely restates a source slide; use `rewrite` or `split` instead.
...
- Look for opportunities to improve the deck's structure, not just its individual slides: if a dense slide would be clearer as two focused slides, use `split`; if the narrative jumps between sections with no transition or ends without a recap, use `add`. A good remix often changes the slide count — do not treat the source deck's structure as fixed.
