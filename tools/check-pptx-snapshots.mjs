...
async function main() {
  ...
  try {
    ...
    // Orphaned expectations (fixture renamed/deleted) would otherwise sit
    // forever, giving the impression of coverage that no longer exists.
    const expectedFiles = fs.existsSync(EXPECTED_DIR) ? fs.readdirSync(EXPECTED_DIR) : [];
    for (const file of expectedFiles) {
      const fixtureName = file.replace(/\.md$/, "");
      if (!fixtures.includes(fixtureName)) {
        console.error(`ORPHAN expectation ${path.join(EXPECTED_DIR, file)} — no matching fixture`);
        failures++;
      }
    }
    ...
    for (const name of fixtures) {
      ...
      if (actual !== expected) {
        // findIndex is -1 when actual is a strict prefix of expected (e.g. a
        // trailing-newline-only change); compare from the end in that case.
        const fromEnd = expected.length - actual.length;
        const i = [...actual].findIndex((ch, j) => ch !== expected[j]);
        const at = i === -1 ? expected.length - 1 : i;
        console.error(
          `DIFF in ${name} at char ${at}${i === -1 ? ` (actual is a prefix; expected has ${fromEnd} more chars)` : ""}\n` +
