...
| [ ] Add "Start Poll" UI               | A button in the presenter panel or header (like the break button). Clicking opens a lightweight modal: choose type (word cloud, multiple choice, open text), enter question text, paste the Slido/Mentimeter embed URL or session code. Quick and frictionless — the presenter does this live, not in advance.                                         |
