...
| [x] Add privacy/security statement | Document where data goes: markdown stays local, AI calls go directly to user-configured OpenRouter/Ollama endpoint, API key stored in localStorage, no telemetry. Essential for public trust. Landed: README "Privacy & Data" section — local-first storage, no telemetry, direct-to-endpoint AI calls with keys kept in browser storage, the Google Fonts PPTX font-metrics exception, and self-contained exports.    |
