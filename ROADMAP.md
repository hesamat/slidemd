...
| [ ] Auto-place viewer on external screen | Use the Window Management API (`getScreenDetails()`) as a progressive enhancement in `RoleManager.togglePresentWindow()`. When available and permitted, detect the external display, open the viewer window positioned/sized to fill it, and fullscreen it on that screen via `requestFullscreen({ screen })`. Fall back to the current manual window flow on Firefox/Safari or if permission is denied. |
