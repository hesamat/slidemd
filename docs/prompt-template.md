...
1. **Outline phase** — `reimagine-outline-prompt.md` asks the AI to read the deck summary (and optionally images) and propose a `{ plan, visualSystem: { visualDirection }, chapters: [{title, flowTag, summary, slides: [{title, intent}]}] }` JSON. The `plan` is a 1-3 sentence statement combining the core message, fresh editorial angle, and chosen narrative structure. The `visualDirection` is a freeform string describing the mood and rules of thumb for choosing backgrounds and layouts. Chapters group slides into a narrative arc (3-7 chapters, each with a flow tag from a fixed vocabulary: hook, context, problem, tension, solution, evidence, comparison, example, transition, climax, cta). The prompt includes a slide-count guard targeting 70-120% of the source deck.
2. **User review** — `AiReimagineOutlineModal` shows the `plan`, `visualDirection`, and chapter-grouped outline. The plan and visual direction are editable textareas. Chapters are collapsible rows with colored flow badges; an "Edit" toggle reveals inputs for editing chapters and slides. The user can continue or cancel.
3. **Generate phase** — the edited outline is flattened into a virtual deck of brief-only slides (`<!-- brief: {title} — {intent} (chapter: ...) | beat: ..., energy: ..., contrast: ..., relationship: ... | image: ... -->`) and run through `generate-prompt.md`. The breakdown AI assigns each slide a `visualBeat` (continuation, transition, punctuation, emotional, divider) plus energy/contrast/relationship modifiers. The generate prompt receives the `visualDirection` via `buildVisualSystemBrief` in the options suffix and the `present` variant of `visual-styling-note.md`, which provides beat-to-treatment mapping, background validation rules, and theme/background pairing guidance. Phase 2 sees only the outline, not the original deck, so content is generated fresh. `applyVisualSystemIdentity` post-processes the output to infer missing `theme:` values, reject invalid backgrounds, and drop stray comments.
