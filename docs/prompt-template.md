...
### Reimagine (outline → review → breakdown → generate)

Uses a dedicated four-phase flow separate from Remix:

1. **Outline phase** — `reimagine-outline-prompt.md` asks the AI to read the deck summary (and optionally images) and propose a `{ plan, visualSystem: { visualDirection }, keepImages, firstSlideIdentity, chapters: [{title, flowTag, summary, suggestedSlideCount}] }` JSON.
   - The `plan` is a 1-3 sentence statement combining the core message, fresh editorial angle, and chosen narrative structure.
   - The `visualDirection` is a freeform string describing the mood and rules of thumb for choosing backgrounds and layouts.
   - `keepImages` is an optional array of 0-based image indices worth carrying over.
   - `firstSlideIdentity` extracts identifying info (course code, term, author) from the original first slide.
   - Chapters group slides into a narrative arc (3-7 chapters, each with a flow tag from an extended vocabulary: `hook`, `context`, `problem`, `tension`, `solution`, `evidence`, `comparison`, `example`, `transition`, `climax`, `cta`, `objectives`, `steps`, `practice`, `recap`, `assertion`, `implication`).
   - The prompt includes a slide-count guard targeting 70-120% of the source deck.
   - The technique menu is flow-specific: instructional decks see "Objectives → Step-by-step → Examples → Check → Recap", technical decks see "Assertion → Evidence" and "Context → Concept → Evidence → Implications", story decks see "Hook → Tension → Resolution" and "Character arc", persuasive decks see "Problem → Solution → Benefits". The AI may combine techniques — the flow biases the menu, it doesn't lock it. Techniques are injected via `reimagine-flow-techniques.md` variant selection.

2. **User review** — `AiReimagineOutlineModal` shows the `plan`, `visualDirection`, and chapter-grouped outline. The plan and visual direction are editable textareas. Chapters are collapsible rows with colored flow badges; an "Edit" toggle reveals inputs for editing chapter titles, summaries, flow tags, and suggested slide counts. The flow tag dropdown includes all 17 tags. The user can continue, cancel, or regenerate chapters from an edited plan.

3. **Breakdown phase** — `reimagine-breakdown-prompt.md` receives the finalized chapters (serialized as JSON with `title`, `flowTag`, `summary`, `suggestedSlideCount`) and the `visualDirection`. It produces per-slide briefs: `{ title, intent, visualBeat, energy, contrast, relationship, imageQuery? }`. The prompt explains all flow tags and how they should inform beat assignment and density (e.g., `objectives` favors a clear focal point, `steps` favors continuation beats with consistent density, `assertion` pairs naturally with an adjacent `evidence` chapter). `visualBeat` is one of `continuation`, `transition`, `punctuation`, `emotional`, `divider`. `imageQuery` is only allowed as a `reuse:<path>` reference to a kept image. The breakdown AI does not change chapter titles, summaries, flow tags, or ordering.

4. **Generate phase** — the breakdown output is flattened into a virtual deck of brief-only slides (`<!-- brief: {title} — {intent} (chapter: ...) | beat: ..., energy: ..., contrast: ..., relationship: ... | image: ... -->`) and run through `generate-prompt.md`. Beat normalization (`beat-normalizer.js`) catches obvious issues: the first slide is forced to `continuation`, and consecutive high-impact beats (`punctuation`, `emotional`, `divider`) are downgraded to `continuation` to preserve rhythm. The generate prompt receives the `visualDirection` via `buildVisualSystemBrief` in the options suffix and the `present` variant of `visual-styling-note.md`, which provides beat-to-treatment mapping, background validation rules, and theme/background pairing guidance. Phase 4 sees only the outline briefs, not the original deck, so content is generated fresh.

5. **Post-processing** — `applyVisualSystemIdentity` normalizes the output: infers missing `theme:` values from `background:` colors, rejects invalid backgrounds (named CSS colors, `transparent`, `none`) and replaces them with fallbacks, drops stray AI-generated comments, and ensures every slide has both `theme:` and `background:`. `stripFabricatedImages` removes any image references not in the kept set. The output validator emits warning-level checks (`VISUAL_SYSTEM_INVALID_THEME`, `VISUAL_SYSTEM_INVALID_BACKGROUND`, `VISUAL_SYSTEM_IGNORED`) when a visual system is present — these are advisory and logged to the sidebar, not error-level retries.
