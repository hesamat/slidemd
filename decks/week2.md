...
<!-- notes: Welcome back. Last week: Agile/Scrum + basic Python. Today we start designing code like an engineer: we’ll use OOP features, but focus on OO *design* (how to structure responsibilities). -->
...
<!-- notes: Anchor week 2 in the overall course. Show students OOD is not an isolated topic; it's the foundation for requirements, architecture, testing, integration, networking, and security. -->
...
<!-- notes: Read these out loud. Set expectations: we won't memorize keywords; we’ll make design decisions and justify them. -->
...
<!-- notes: This is the critical shift. Students write "Scripts". We need them to write "Modules". -->
...
```d2
direction: down
Global Data: User List
Func A: Login()
Func B: DeleteUser()
Func C: SendEmail()

Global Data -> Func A
Func A -> Global Data : Modifies
Global Data -> Func B
Func B -> Global Data : Deletes
Global Data -> Func C : Crashes (Data missing)
```
...
- **1960s (The Origin):** *Simula 67* (Norway). Researchers needed to simulate explosive complexity in ship designs. Linear code failed; they created "virtual ships" (Classes) to group data and behavior.
- **1970s (The Vision):** Alan Kay (*Smalltalk*) coined the term "Object-Oriented." 
  - He envisioned software like **biological cells**: *independent units protecting their own state, communicating only via messages*.
- **The Shift:** We stopped telling the computer *steps* and started describing *models*.
...
1950s -> 1967: Complexity Crisis
1967 -> 1972: The Paradigm Shift
```


---

layout: "header header" auto "main media" 1fr "code code" 1.75fr / 1fr 1fr
...
@code
<div style="margin-left: 14rem; margin-right: 14rem;">
...
<!-- notes: Students often jump to classes too early. This slide sets a “modular-first” baseline. -->
...
layout: "oop ood" "oop-main ood-main" / 1fr 1fr

<!-- notes: Distinguish between the "Tool" and the "Thinking". Syntax is easy; Design is hard. Emphasize that knowing syntax doesn't make you an engineer, just like knowing grammar doesn't make you a novelist. -->
...
@oop-main
> - **Keywords:** `class`, `self`, `__init__` are just mechanisms to define state.
...
**Requirements:**
1.  **`__init__`**: Should accept `name` (str) and `healing_power` (int).
2.  **`drink()`**: A method that prints: *"You drank [name] and recovered [healing_power] HP!"*
...
<!-- notes: Invariants are rules that must ALWAYS be true. This is the #1 reason to use a class. -->
...
- *e.g., A bank balance cannot be negative.*
- *e.g., A Smart Meter ID cannot be empty.*

If you use a simple Dictionary, anyone can break the rules: <code class="language-python">meter["id"] = ""</code> *(System breaks later)*.

<br>

### **Q:** What does <code class="language-python">raise</code> do?
...
class SmartMeter:
    ...
    def add_reading(self, val: float):
        if val < 0:
            raise ValueError("Invalid volts")
        self._readings.append(val)
```
*Now, it is mathematically impossible for a Meter to exist in a bad state.*

---

layout: "header header" "main media" / 1fr 1fr
<!-- Note:     # Getter method to access private data
...
@main
In other languages (Java/C++), you have specific keywords. Python uses **Naming Conventions**.

- **Public (`name`):** Accessible everywhere.
- **Protected (`_name`):** "Internal use only." Developers should respect this.
- **Private (`__name`):** Python mangles the name (internally renames that variable to `_ClassName__variableName`) to make it harder to access.
...
class Account:
    ...
    # Task: Implemet getter and setter (deposit) methods for balance

a = Account()
print(a._balance) # Works, but bad practice
# print(a.__pin)  # Crashes (AttributeError)
```
...
<!-- notes: 
This exercise checks if they understand State and Invariants.
Stop the class and ask: 
1. "What data do we need?" (balance, stock)
2. "What are the rules?" (balance >= price, stock > 0)
...
**Your Task:**
1.  List the **Attributes** (What data does the object need to remember?).
2.  List the **Methods** (What actions can the user take?).
3.  Identify the **Invariants** (What rules must *never* be broken?).
...
<!-- notes: Distributed systems rely on Interfaces. "I don't care who you are, as long as you speak JSON." -->
...
<!-- notes: Emphasize: Polymorphism is about calling through a stable interface, not about inheritance. -->
...
<!-- notes: Composition is key for distributed systems. A "System" is composed of "Modules". -->
...
<!-- notes: 
The classic Joe Armstrong quote: "You wanted a banana but you got a gorilla holding the banana and the entire jungle."
Inheritance couples you to the internals of the parent. If the parent changes, the child breaks (Fragile Base Class).
-->
...
**Why?**
1.  **IDE Support:** Autocomplete works better.
2.  **Debugging:** Catch type errors *before* running code.
3.  **Documentation:** Explicitly states what data is expected.
...
@main
In a team, you write code for *humans*, not just computers.

**Google Style Guide:**
1.  **Summary:** One line description.
2.  **Args:** Parameters and their purpose.
3.  **Returns:** What comes back.
4.  **Raises:** What errors might happen.
...
layout: "header header" "main media" / 1fr 1fr
<!-- notes: 
"""Retrieves a specific theme setting from the nested user profile dictionary.

    Args:
        user_profile: A dictionary containing user account data and settings.
        setting_name: The name of the specific theme configuration to retrieve.

    Returns:
        The theme string found in settings or default_light if the setting is missing. 
        Returns None if the settings dictionary does not exist at all.
    """ 
-->
...
1.  Add **Type Hints** (`data` is a dict, `key` is a string).
2.  Add a **Google-Style Docstring**.
3.  Add the **Return Type** (it returns a string or None).
...
If you print an object and see `<Object at 0x7f...>`, you can't debug.
...
<!-- notes: 
Code crashes happen. In systems, we can't just let the program stop because one file is missing.
Explain the 3 main keywords:
1. TRY: Run this code, but watch out for errors.
2. EXCEPT: If a specific error happens, run this code instead of crashing.
3. FINALLY: Run this no matter what (good for cleaning up).
-->
...
1.  **`try`**: Attempt the dangerous code.
2.  **`except`**: Define what to do if it fails (e.g., use a default value).
3.  **`finally`**: Cleanup code that **always** runs (like closing files), even if an error occurred.
...
The error they need to catch is 'IndexError'.
Make sure they don't just delete the code, but wrap it in the try/except block.
-->
...
**Your Task:**
1.  Wrap the dangerous line in a `try` block.
2.  Catch the `IndexError`.
3.  If caught, print "User not found" and return `None`.
4.  Add a `finally` block that prints "Lookup finished".
...
<!-- notes: End with the theme: design for change. Then preview next week (requirements) and connect. -->
...
- Define clear **interfaces** to separate *what* a system does from *how* it does it
